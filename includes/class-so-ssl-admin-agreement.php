...
// If this file is called directly, abort.
	...
	class So_SSL_Admin_Agreement
	{
		...
		public static function init()
		{
			...
			add_action('admin_init', array(
				__CLASS__,
				'check_admin_agreement'
			), 20);

			// Add AJAX handler for saving agreement
			add_action('wp_ajax_so_ssl_save_admin_agreement', array(
				__CLASS__,
				'ajax_save_admin_agreement'
			));

			// Add notice on plugins page to inform admins about the emergency override
			add_action('admin_notices', array(
				__CLASS__,
				'maybe_show_emergency_notice'
			));
		}
    ...
    ";
		...
		public static function add_agreement_menu()
		{
			...
			remove_submenu_page('so-ssl-agreement', 'so-ssl-agreement');
		}
            ...
            <div class="wrap so-ssl-agreement-wrap">
                <div class="so-ssl-agreement-container">
                    ...
                    <div class="so-ssl-agreement-content">
						<?php
							echo wp_kses_post($agreement_text); ?>
                    </div>

                    <div class="so-ssl-agreement-form">
                        <form id="so-ssl-admin-agreement-form">
							<?php
								wp_nonce_field('so_ssl_admin_agreement', 'so_ssl_admin_agreement_nonce'); ?>
                            ...
                            <div class="so-ssl-agreement-checkbox">
                                <label>
                                    <input type="checkbox"
                                           id="so_ssl_admin_agreement_accept"
                                           name="so_ssl_admin_agreement_accept"
                                           value="1" required>
									<?php
                            ...
                            <div class="so-ssl-agreement-actions">
                                <button type="submit" id="so_ssl_agreement_submit"
                                        class="button button-primary" disabled>
									<?php
										esc_html_e('Accept and Continue', 'so-ssl'); ?>
                                </button>
                                ...
                                <a href="<?php
                                   ...
                                   class="so-ssl-alternate-button">
									<?php
										esc_html_e('Disagree and Go Back', 'so-ssl'); ?>
                                </a>
                            </div>

                            <div id="so-ssl-agreement-message"
                                 class="so-ssl-agreement-message"
                    ...
                    <div class="so-ssl-agreement-options">
                        <p>
							<?php
								esc_html_e('Need to disable this agreement feature?', 'so-ssl'); ?>
                        </p>
                        <a href="<?php
							echo esc_url(admin_url('index.php&disable_so_ssl_agreement=1')); ?>"
                           class="so-ssl-emergency-button">
                        <span class="dashicons dashicons-dismiss"
                              style="margin-right: 4px; margin-top: 3px;"></span>
							<?php
								esc_html_e('Disable Admin Agreement', 'so-ssl'); ?>
                        </a>
                        <a href="<?php
							echo esc_url(admin_url('options-general.php')); ?>"
                           class="so-ssl-emergency-button">
                        <span class="dashicons dashicons-undo"
                              style="margin-right: 4px; margin-top: 3px;"></span>
							<?php
								esc_html_e('Return to Settings', 'so-ssl'); ?>
                        </a>
                    </div>
                </div>
            </div>
