...
class So_SSL_Two_Factor {
	...
	public static function add_2fa_user_fields( $user ) {
        ...
        <div class="so-ssl-2fa-instructions"
             style="background: #f8f9fa; border-left: 4px solid #2271b1; padding: 15px; margin-bottom: 20px;">
        ...
        <table class="form-table">
            <tr>
                <th>
                    <label for="so_ssl_2fa_enabled"><?php esc_html_e( 'Enable Two-Factor Authentication', 'so-ssl' ); ?></label>
                </th>
                <td>
                    <input type="checkbox" name="so_ssl_2fa_enabled"
                           id="so_ssl_2fa_enabled"
			...
			<?php if ( $method === 'authenticator' ): ?>
                <tr id="so_ssl_2fa_authenticator_row"
			...
			<?php else: ?>
                <tr id="so_ssl_2fa_email_row"
                    style="<?php echo esc_attr( ( $enabled !== '1' ) ? 'display:none;' : '' ); ?>">
                    <th><?php esc_html_e( 'Email Authentication', 'so-ssl' ); ?></th>
                    <td>
                        <div class="so-ssl-email-2fa-info">
                            <p>
                                <strong><?php esc_html_e( 'How Email Verification Works:', 'so-ssl' ); ?></strong>
                            </p>
            ...
            <tr id="so_ssl_2fa_backup_codes_row"
                style="<?php echo ( $enabled !== '1' ) ? 'display:none;' : ''; ?>">
                <th><?php esc_html_e( 'Backup Codes', 'so-ssl' ); ?></th>
                <td>
                    <button type="button" id="so_ssl_generate_backup_codes"
                            class="button"><?php esc_html_e( 'Generate Backup Codes', 'so-ssl' ); ?></button>
                    <div id="so_ssl_backup_codes_container"
                        ...
                        <div id="so_ssl_backup_codes_actions"
                             style="margin-top: 15px;">
                            <label>
                                <input type="checkbox"
                                       id="so_ssl_email_backup_codes"
                                       checked="checked"/>
								<?php esc_html_e( 'Email backup codes to:', 'so-ssl' ); ?>
                                <strong><?php echo esc_html( $user->user_email ); ?></strong>
                            </label>
                            <button type="button" id="so_ssl_send_backup_codes"
                                    class="button"
            ...
            <div class="so-ssl-setup-steps">
                ...
                <div class="so-ssl-step">
                    ...
                    <div class="so-ssl-qr-code-container">
                        ...
                        <div id="so-ssl-qr-code"
                             data-totp-url="<?php echo esc_attr( $totp_url ); ?>"></div>
                    ...
                    <details class="so-ssl-manual-entry">
                        ...
                        <div class="so-ssl-manual-code">
                            ...
                            <button type="button" class="so-ssl-copy-btn"
                                    data-copy="<?php echo esc_attr( $user_identifier ); ?>">
                        ...
                        <div class="so-ssl-manual-code">
                            ...
                            <button type="button" class="so-ssl-copy-btn"
                                    data-copy="<?php echo esc_attr( $secret ); ?>">
                                <span class="dashicons dashicons-clipboard"></span>
                            </button>
                        </div>
                        <div class="so-ssl-manual-code">
                ...
                <div class="so-ssl-step">
                    ...
                    <div class="so-ssl-verify-code">
                        ...
                        <button type="button" id="so_ssl_verify_code_button"
                                class="button button-primary">
							<?php esc_html_e( 'Verify Code', 'so-ssl' ); ?>
                        </button>

                        <div id="so_ssl_verify_result"
                             class="so-ssl-verify-result"></div>
                    </div>
                </div>
            </div>
	...
	public static function add_2fa_login_fields() {
		...
		if ( $requires_2fa ) {
			...
			?>
            <p>
                <label for="so_ssl_2fa_code"><?php esc_html_e( 'Authentication Code', 'so-ssl' ); ?>
                    <br/>
                    <input type="text" name="so_ssl_2fa_code"
                           id="so_ssl_2fa_code" class="input" size="20"
                           autocomplete="off"/>
                </label>
            </p>
			<?php
...
add_action( 'wp_ajax_so_ssl_generate_backup_codes', array(
	'So_SSL_Two_Factor',
	'generate_backup_codes'
) );
add_action( 'wp_ajax_so_ssl_verify_totp_code', array(
	'So_SSL_Two_Factor',
	'verify_totp_code'
) );
// Register AJAX handlers
...
add_action( 'wp_ajax_so_ssl_email_backup_codes', array(
	'So_SSL_Two_Factor',
	'ajax_email_backup_codes'
) ); // Add this line
