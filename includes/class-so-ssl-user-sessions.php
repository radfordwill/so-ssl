...
class So_SSL_User_Sessions {
	...
	public static function init() {
		...
		add_action( 'show_user_profile', array(
			__CLASS__,
			'add_sessions_management_profile'
		) );
		...
		add_action( 'wp_ajax_so_ssl_terminate_session', array(
			__CLASS__,
			'ajax_terminate_session'
		) );
		add_action( 'wp_ajax_so_ssl_terminate_all_sessions', array(
			__CLASS__,
			'ajax_terminate_all_sessions'
		) );
		add_action( 'wp_ajax_so_ssl_terminate_other_sessions', array(
			__CLASS__,
			'ajax_terminate_other_sessions'
		) );

		// Hook to check session expiry on auth cookie expiration
		add_action( 'auth_cookie_expired', array(
			__CLASS__,
		...
		add_action( 'so_ssl_cleanup_expired_sessions', array(
			__CLASS__,
            ...
            <tr>
                ...
                <td>
					...
					<?php if ( current_user_can( 'manage_options' ) ): ?>
                        <p>
                            <a href="<?php sprintf( admin_url( 'options-general.php?page=so-ssl#login-limit' ) );
							?>"><?php
								/* translators: %s: Title' */
                    ...
                    <div class="so-ssl-session-actions">
                        <button type="button"
                                id="so_ssl_terminate_other_sessions"
                                class="button"><?php esc_html_e( 'Terminate All Other Sessions', 'so-ssl' ); ?></button>
                        <button type="button" id="so_ssl_terminate_all_sessions"
                                class="button"><?php esc_html_e( 'Terminate All Sessions', 'so-ssl' ); ?></button>
                    </div>
                        ...
                        <table class="widefat so-ssl-sessions-table">
							...
							<?php foreach ( $sessions as $token => $session ): ?>
                                <tr class="<?php echo ( $token === $current_session ) ? 'so-ssl-current-session' : ''; ?>">
                                    ...
                                    <td>
                                        <button type="button"
                                                class="button button-small so-ssl-terminate-session"
					...
					<?php foreach ( $users_with_sessions as $user_id => $user_data ): ?>
                        <div class="so-ssl-user-sessions-container">
                            <h3>
                                ...
                                <button type="button"
                                        class="button button-small so-ssl-terminate-all-user-sessions"
