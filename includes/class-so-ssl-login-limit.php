...
class So_SSL_Login_Limit {
	...
	public static function init() {
		...
		add_action( 'admin_menu', array(
			__CLASS__,
			'add_login_limit_menu'
		), 90 );

		// Add AJAX handler for whitelist/blacklist management
		add_action( 'wp_ajax_so_ssl_whitelist_ip', array(
			__CLASS__,
			'ajax_whitelist_ip'
		) );
		add_action( 'wp_ajax_so_ssl_blacklist_ip', array(
			__CLASS__,
			'ajax_blacklist_ip'
		) );
		add_action( 'wp_ajax_so_ssl_remove_from_list', array(
			__CLASS__,
			'ajax_remove_from_list'
		) );
		add_action( 'wp_ajax_so_ssl_reset_attempts', array(
			__CLASS__,
			'ajax_reset_attempts'
		) );
		...
		add_action( 'so_ssl_cleanup_login_attempts', array(
			__CLASS__,
	...
	public static function display_login_limit_page() {
        ...
        <div class="so-ssl-login-limit-tabs">
            <div class="nav-tab-wrapper">
                <a href="#settings"
                   class="nav-tab nav-tab-active"><?php /* translators: Tab title for settings section */
					esc_html_e( 'Settings', 'so-ssl' ); ?></a>
                <a href="#current-lockouts"
                   class="nav-tab"><?php /* translators: Tab title for current lockouts section */
					esc_html_e( 'Current Lockouts', 'so-ssl' ); ?></a>
                <a href="#statistics"
                   class="nav-tab"><?php /* translators: Tab title for statistics section */
					esc_html_e( 'Statistics', 'so-ssl' ); ?></a>
                <a href="#whitelist"
                   class="nav-tab"><?php /* translators: Tab title for whitelist section */
					esc_html_e( 'Whitelist', 'so-ssl' ); ?></a>
                <a href="#blacklist"
                   class="nav-tab"><?php /* translators: Tab title for blacklist section */
					esc_html_e( 'Blacklist', 'so-ssl' ); ?></a>
                <a href="#login-history"
                   class="nav-tab"><?php /* translators: Tab title for login history section */
					esc_html_e( 'Login History', 'so-ssl' ); ?></a>
            </div>

            <div id="settings" class="tab-content active">
                <h2><?php /* translators: Heading for login limit settings section */
					esc_html_e( 'Login Limiting Settings', 'so-ssl' ); ?></h2>
            ...
            <div id="current-lockouts" class="tab-content">
                <h2><?php /* translators: Heading for current lockouts section */
					esc_html_e( 'Current Lockouts', 'so-ssl' ); ?></h2>
				<?php if ( empty( $attempts ) ): ?>
                    <p><?php /* translators: Message shown when no login attempts are recorded */
						esc_html_e( 'No login attempts recorded.', 'so-ssl' ); ?></p>
				<?php else: ?>
                    <table class="widefat striped">
                        <thead>
                        <tr>
                            <th><?php /* translators: Column header for IP address */
								esc_html_e( 'IP Address', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for failed attempts count */
								esc_html_e( 'Failed Attempts', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for lockouts count */
								esc_html_e( 'Lockouts', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for attempted usernames */
								esc_html_e( 'Attempted Usernames', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for last attempt time */
								esc_html_e( 'Last Attempt', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for lockout status */
								esc_html_e( 'Status', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for available actions */
								esc_html_e( 'Actions', 'so-ssl' ); ?></th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
									...
									if ( ! empty( $data['usernames'] ) ) {
										...
										/* translators: Text shown when no usernames were attempted */
										esc_html_e( 'None', 'so-ssl' );
									}
									?>
                                </td>
                                ...
                                <td>
                                    <button type="button"
                                            class="button so-ssl-whitelist-ip"
                                            data-ip="<?php echo esc_attr( $ip ); ?>"><?php /* translators: Button text to whitelist an IP */
										esc_html_e( 'Whitelist', 'so-ssl' ); ?></button>
                                    <button type="button"
                                            class="button so-ssl-blacklist-ip"
                                            data-ip="<?php echo esc_attr( $ip ); ?>"><?php /* translators: Button text to blacklist an IP */
										esc_html_e( 'Blacklist', 'so-ssl' ); ?></button>
                                    <button type="button"
                                            class="button so-ssl-reset-attempts"
                                            data-ip="<?php echo esc_attr( $ip ); ?>"><?php /* translators: Button text to reset attempts for an IP */
										esc_html_e( 'Reset', 'so-ssl' ); ?></button>
                                </td>
                            </tr>
						<?php endforeach; ?>
                        </tbody>
            ...
            <div id="statistics" class="tab-content">
                <h2><?php /* translators: Heading for login statistics section */
					esc_html_e( 'Login Attempt Statistics', 'so-ssl' ); ?></h2>
				<?php
                ...
                <div class="so-ssl-stats-grid">
                    <div class="so-ssl-stat-box">
                        <h3><?php /* translators: Label for failed attempts count */
							esc_html_e( 'Failed Attempts', 'so-ssl' ); ?></h3>
                        <div class="so-ssl-stat-number"><?php echo esc_html( $total_failed ); ?></div>
                    </div>

                    <div class="so-ssl-stat-box">
                        <h3><?php /* translators: Label for successful logins count */
							esc_html_e( 'Successful Logins', 'so-ssl' ); ?></h3>
                        <div class="so-ssl-stat-number"><?php echo esc_html( $total_success ); ?></div>
                    </div>

                    <div class="so-ssl-stat-box">
                        <h3><?php /* translators: Label for total lockouts count */
							esc_html_e( 'Total Lockouts', 'so-ssl' ); ?></h3>
                        <div class="so-ssl-stat-number"><?php echo esc_html( $total_lockouts ); ?></div>
                    </div>

                    <div class="so-ssl-stat-box">
                        <h3><?php /* translators: Label for unique IPs count */
							esc_html_e( 'Unique IPs', 'so-ssl' ); ?></h3>
                ...
                <div class="so-ssl-stats-tables">
                    <div class="so-ssl-stats-table">
                        <h3><?php /* translators: Heading for most targeted usernames table */
							esc_html_e( 'Most Targeted Usernames', 'so-ssl' ); ?></h3>
						<?php if ( empty( $targeted_usernames ) ): ?>
                            <p><?php /* translators: Message shown when no username data is available */
								esc_html_e( 'No data available', 'so-ssl' ); ?></p>
						<?php else: ?>
                            <table class="widefat striped">
                                <thead>
                                <tr>
                                    <th><?php /* translators: Column header for username */
										esc_html_e( 'Username', 'so-ssl' ); ?></th>
                                    <th><?php /* translators: Column header for failed attempts count */
										esc_html_e( 'Failed Attempts', 'so-ssl' ); ?></th>
                                </tr>
                                </thead>
            ...
            <div id="whitelist" class="tab-content">
                <h2><?php /* translators: Heading for IP whitelist section */
					esc_html_e( 'IP Whitelist', 'so-ssl' ); ?></h2>
                <p><?php /* translators: Description of whitelist functionality */
					esc_html_e( 'Whitelisted IPs are exempt from login limiting.', 'so-ssl' ); ?></p>

                <div class="so-ssl-add-ip-form">
                    <h3><?php /* translators: Heading for adding IP to whitelist form */
						esc_html_e( 'Add IP to Whitelist', 'so-ssl' ); ?></h3>
                    <form id="so-ssl-add-whitelist-form">
                        <input type="text" id="so-ssl-new-whitelist-ip"
                               placeholder="<?php esc_attr_e( 'Enter IP address', 'so-ssl' ); ?>"/>
                        <button type="submit"
                                class="button"><?php /* translators: Button text to add IP to whitelist */
							esc_html_e( 'Add to Whitelist', 'so-ssl' ); ?></button>
                    </form>
                </div>

				<?php if ( empty( $whitelist ) ): ?>
                    <p><?php /* translators: Message shown when whitelist is empty */
						esc_html_e( 'No IPs are currently whitelisted.', 'so-ssl' ); ?></p>
				<?php else: ?>
                    <table class="widefat striped">
                        <thead>
                        <tr>
                            ...
                            <th><?php /* translators: Column header for date added */
								esc_html_e( 'Added', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for available actions */
								esc_html_e( 'Actions', 'so-ssl' ); ?></th>
                        </tr>
                        </thead>
						...
						<?php foreach ( $whitelist as $ip => $data ): ?>
                            <tr>
                                ...
                                <td>
                                    <button type="button"
                                            class="button so-ssl-remove-from-list"
                                            data-ip="<?php echo esc_attr( $ip ); ?>"
                                            data-list="whitelist"><?php /* translators: Button text to remove IP from whitelist */
										esc_html_e( 'Remove', 'so-ssl' ); ?></button>
                                </td>
                            </tr>
						<?php endforeach; ?>
                        </tbody>
            ...
            <div id="blacklist" class="tab-content">
                <h2><?php /* translators: Heading for IP blacklist section */
					esc_html_e( 'IP Blacklist', 'so-ssl' ); ?></h2>
                <p><?php /* translators: Description of blacklist functionality */
					esc_html_e( 'Blacklisted IPs are permanently blocked from logging in.', 'so-ssl' ); ?></p>

                <div class="so-ssl-add-ip-form">
                    <h3><?php /* translators: Heading for adding IP to blacklist form */
						esc_html_e( 'Add IP to Blacklist', 'so-ssl' ); ?></h3>
                    <form id="so-ssl-add-blacklist-form">
                        <input type="text" id="so-ssl-new-blacklist-ip"
                               placeholder="<?php esc_attr_e( 'Enter IP address', 'so-ssl' ); ?>"/>
                        <button type="submit"
                                class="button"><?php /* translators: Button text to add IP to blacklist */
							esc_html_e( 'Add to Blacklist', 'so-ssl' ); ?></button>
                    </form>
                </div>

				<?php if ( empty( $blacklist ) ): ?>
                    <p><?php /* translators: Message shown when blacklist is empty */
						esc_html_e( 'No IPs are currently blacklisted.', 'so-ssl' ); ?></p>
				<?php else: ?>
                    <table class="widefat striped">
                        <thead>
                        <tr>
                            ...
                            <th><?php /* translators: Column header for date added */
								esc_html_e( 'Added', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for blacklist reason */
								esc_html_e( 'Reason', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for available actions */
								esc_html_e( 'Actions', 'so-ssl' ); ?></th>
                        </tr>
                        </thead>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button type="button"
                                            class="button so-ssl-remove-from-list"
                                            data-ip="<?php echo esc_attr( $ip ); ?>"
                                            data-list="blacklist"><?php /* translators: Button text to remove IP from blacklist */
										esc_html_e( 'Remove', 'so-ssl' ); ?></button>
                                </td>
                            </tr>
						<?php endforeach; ?>
            ...
            <div id="login-history" class="tab-content">
                <h2><?php /* translators: Heading for login history section */
					esc_html_e( 'Login History', 'so-ssl' ); ?></h2>

				<?php if ( empty( $history ) ): ?>
                    <p><?php /* translators: Message shown when no login history is recorded */
						esc_html_e( 'No login history recorded.', 'so-ssl' ); ?></p>
				<?php else: ?>
                    <table class="widefat striped">
                        <thead>
                        <tr>
                            <th><?php /* translators: Column header for login time */
								esc_html_e( 'Time', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for IP address */
								esc_html_e( 'IP Address', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for username */
								esc_html_e( 'Username', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for login status */
								esc_html_e( 'Status', 'so-ssl' ); ?></th>
                            <th><?php /* translators: Column header for browser info */
								esc_html_e( 'Browser', 'so-ssl' ); ?></th>
                        </tr>
                        </thead>
                        <tbody>
						<?php
						...
						foreach ( $history as $entry ):
							if ( $count ++ > 100 ) {
								break;
							} // Show only 100 most recent entries
							?>
                            <tr>
	...
	public static function ajax_remove_from_list() {
		...
		if ( empty( $ip ) || ! in_array( $list, array(
				'whitelist',
				'blacklist'
			) ) ) {
