...
class So_SSL {
	...
	private function define_public_hooks() {
		...
		add_action( 'send_headers', array(
			$this,
			'add_csp_frame_ancestors_header'
		) );
		add_action( 'send_headers', array(
			$this,
			'add_referrer_policy_header'
		) );
		add_action( 'send_headers', array(
			$this,
			'add_content_security_policy_header'
		) );
		add_action( 'send_headers', array(
			$this,
		...
		add_action( 'send_headers', array(
			$this,
			'add_cross_origin_policy_headers'
		) );
		...
		if ( get_option( 'so_ssl_disable_weak_passwords', 0 ) ) {
			...
			add_action( 'validate_password_reset', array(
				$this,
	...
	public function display_options_page() {
        ...
        <div class="wrap so-ssl-wrap">
            <div class="so-ssl-header">
                <h1>
                    <svg viewBox="0 0 500 300"
                        ...
                        <!-- Background Shield -->
                        <path d="M250 30 L380 80 L380 170 C380 240 320 270 250 290 C180 270 120 240 120 170 L120 80 Z"
                              fill="#f1f5f9" stroke="#334155" stroke-width="8"/>

                        <!-- So Text -->
                        <text x="250" y="100" font-family="Arial, sans-serif"
                              font-size="40" font-weight="bold"
                              text-anchor="middle" fill="#0ea5e9">So
                        </text>

                        <!-- SSL Text -->
                        <text x="250" y="160" font-family="Arial, sans-serif"
                              font-size="80" font-weight="bold"
                              text-anchor="middle" fill="#334155">SSL
                        </text>

                        <!-- 3D Lock - positioned higher on the shield -->
                        <g transform="translate(335, 250) rotate(-25) scale(1.5)">
                            <!-- Lock Body Shadow (for 3D effect) -->
                            <rect x="-22" y="0" width="44" height="32" rx="5"
                                  ry="5" fill="#0c4a6e" opacity="0.5"/>

                            <!-- Lock Body -->
                            <rect x="-20" y="-2" width="40" height="30" rx="5"
                                  ry="5" fill="#0ea5e9"/>

                            <!-- Lock Body Highlight (for 3D effect) -->
                            <rect x="-18" y="-2" width="36" height="5" rx="2"
                                  ry="2" fill="#7dd3fc" opacity="0.5"/>

                            <!-- Lock Shackle Behind Shield (for 3D effect) -->
                            <path d="M-10 -2 L-10 -14 C-10 -23 10 -23 10 -14 L10 -2"
                                  fill="none" stroke="#64748b" stroke-width="6"
                                  stroke-linecap="round"/>

                            <!-- Lock Shackle Front (for 3D effect) -->
                            <path d="M-10 -2 L-10 -15 C-10 -25 10 -25 10 -15 L10 -2"
                                  fill="none" stroke="#334155" stroke-width="6"
                                  stroke-linecap="round"/>

                            <!-- Shackle Highlights (for 3D effect) -->
                            <path d="M-10 -15 C-10 -24 10 -24 10 -15"
                            ...
                            <rect x="-1.5" y="13" width="3" height="7"
                                  fill="#64748b"/>

                            <!-- Keyhole Highlight (for 3D effect) -->
                            <path d="M-3 10 A5 5 0 0 1 3 10" stroke="#f1f5f9"
                                  stroke-width="1" fill="none" opacity="0.7"/>
                        </g>

                        <!-- Security Dots -->
                        <circle cx="145" cy="210" r="8" fill="#0ea5e9"
                                opacity="0.7"/>
                        <circle cx="355" cy="210" r="8" fill="#0ea5e9"
                                opacity="0.7"/>
                        <circle cx="170" cy="250" r="6" fill="#0ea5e9"
                                opacity="0.5"/>
                        <circle cx="330" cy="250" r="6" fill="#0ea5e9"
                                opacity="0.5"/>
                    </svg>
            ...
            <div class="nav-tab-wrapper">
                <a href="#ssl-settings"
                   class="nav-tab nav-tab-active"><?php esc_html_e( 'SSL Settings', 'so-ssl' ); ?></a>
                <a href="#content-security"
                   class="nav-tab"><?php esc_html_e( 'Content Security', 'so-ssl' ); ?></a>
                <a href="#browser-features"
                   class="nav-tab"><?php esc_html_e( 'Browser Features', 'so-ssl' ); ?></a>
                <a href="#cross-origin"
                   class="nav-tab"><?php esc_html_e( 'Cross-Origin', 'so-ssl' ); ?></a>
                <a href="#two-factor"
                   class="nav-tab"><?php esc_html_e( 'Two-Factor Auth', 'so-ssl' ); ?></a>
                <a href="#login-protection"
                   class="nav-tab"><?php esc_html_e( 'Login Protection', 'so-ssl' ); ?></a>
                <a href="#user-sessions"
                   class="nav-tab"><?php esc_html_e( 'User Sessions', 'so-ssl' ); ?></a>
                <a href="#login-limit"
                   class="nav-tab"><?php esc_html_e( 'Login Limiting', 'so-ssl' ); ?></a>
                <a href="#privacy-compliance"
                   class="nav-tab"><?php esc_html_e( 'Privacy Compliance', 'so-ssl' ); ?></a>
                <a href="#admin-agreement"
                   class="nav-tab"><?php esc_html_e( 'Admin Agreement', 'so-ssl' ); ?></a>
            </div>
                ...
                <div id="two-factor" class="settings-tab">
					...
					<?php if ( get_option( 'so_ssl_enable_2fa', 0 ) ): ?>
                        <div class="so-ssl-notice">
                            <p>
                                <strong><?php esc_html_e( 'Next Steps:', 'so-ssl' ); ?></strong>
                            </p>
                            <ol>
            ...
            <div class="so-ssl-compliance-highlights">
                <div class="so-ssl-compliance-detail">
                    <h4>
                        <span class="dashicons dashicons-welcome-view-site"></span> <?php esc_html_e( 'Customizable Privacy Page', 'so-ssl' ); ?>
                    </h4>
                    <p><?php esc_html_e( 'Create a custom privacy acknowledgment page that users must accept before accessing your site.', 'so-ssl' ); ?></p>
                </div>
                <div class="so-ssl-compliance-detail">
                    <h4>
                        <span class="dashicons dashicons-groups"></span> <?php esc_html_e( 'Role-Based Configuration', 'so-ssl' ); ?>
                    </h4>
                ...
                <div class="so-ssl-compliance-detail">
                    <h4>
                        <span class="dashicons dashicons-calendar-alt"></span> <?php esc_html_e( 'Expiration Controls', 'so-ssl' ); ?>
                    </h4>
                    <p><?php esc_html_e( 'Set acknowledgment expiry periods to ensure users regularly review updated privacy information.', 'so-ssl' ); ?></p>
                </div>
                <a href="<?php echo esc_url( admin_url( 'options-general.php?page=so-ssl#privacy-compliance' ) ); ?>"
	...
	public static function output_flush_rules_button() {
		...
		// Display the button
		?>
        <div class="so-ssl-admin-section"
             style="margin-top: 20px; padding: 15px; background: #f8f9fa; border-left: 4px solid #72aee6;">
            ...
            <form method="post">
                ...
                <input type="submit" class="button button-secondary"
                       value="<?php esc_attr_e( 'Flush Rewrite Rules', 'so-ssl' ); ?>">
            </form>
        </div>
		<?php
	}
	...
	public static function add_flush_rules_button() {
        ...
        <div class="so-ssl-admin-section"
            ...
            <form method="post">
				<?php wp_nonce_field( 'so_ssl_flush_rules', 'so_ssl_flush_rules_nonce' ); ?>
                <input type="submit" class="button button-secondary"
                       value="<?php esc_html_e( 'Flush Rewrite Rules', 'so-ssl' ); ?>">
            </form>
        </div>
		<?php
	}
	...
	public function privacy_compliance_section_callback() {
        ...
        <div>
            <a href="<?php echo esc_url( add_query_arg( esc_attr( $page_slug ), '1', site_url() ) ); ?>"
               target="_blank" class="button button-primary"
               style="font-size: 14px; height: auto; padding: 8px 16px;">
				<?php esc_html_e( 'Open Privacy Page', 'so-ssl' ); ?>
                <span class="dashicons dashicons-external"
                      style="font-size: 16px; height: 16px; width: 16px; vertical-align: text-bottom;"></span>
            </a>
        </div>
	...
	private function register_permissions_policy_settings() {
		...
		$permissions = array(
			'accelerometer'                   => array(
				'default'     => 'self',
				'description' => __( 'Controls access to accelerometer sensors', 'so-ssl' )
			),
			'ambient-light-sensor'            => array(
				'default'     => 'self',
				'description' => __( 'Controls access to ambient light sensors', 'so-ssl' )
			),
			'autoplay'                        => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to autoplay media', 'so-ssl' )
			),
			'battery'                         => array(
				'default'     => 'self',
				'description' => __( 'Controls access to battery information', 'so-ssl' )
			),
			'camera'                          => array(
				'default'     => 'self',
				'description' => __( 'Controls access to video cameras', 'so-ssl' )
			),
			'display-capture'                 => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to capture screen content', 'so-ssl' )
			),
			'document-domain'                 => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to set document.domain', 'so-ssl' )
			),
			'encrypted-media'                 => array(
				'default'     => 'self',
				'description' => __( 'Controls access to EME API', 'so-ssl' )
			),
			'execution-while-not-rendered'    => array(
				'default'     => 'self',
				'description' => __( 'Controls execution when not rendered', 'so-ssl' )
			),
			'execution-while-out-of-viewport' => array(
				'default'     => 'self',
				'description' => __( 'Controls execution when outside viewport', 'so-ssl' )
			),
			'fullscreen'                      => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to use fullscreen mode', 'so-ssl' )
			),
			'geolocation'                     => array(
				'default'     => 'self',
				'description' => __( 'Controls access to the geolocation API', 'so-ssl' )
			),
			'gyroscope'                       => array(
				'default'     => 'self',
				'description' => __( 'Controls access to gyroscope sensors', 'so-ssl' )
			),
			'microphone'                      => array(
				'default'     => 'self',
				'description' => __( 'Controls access to audio capture devices', 'so-ssl' )
			),
			'midi'                            => array(
				'default'     => 'self',
				'description' => __( 'Controls access to MIDI devices', 'so-ssl' )
			),
			'navigation-override'             => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to override navigation', 'so-ssl' )
			),
			'payment'                         => array(
				'default'     => 'self',
				'description' => __( 'Controls access to the Payment Request API', 'so-ssl' )
			),
			'picture-in-picture'              => array(
				'default'     => '*',
				'description' => __( 'Controls the ability to use Picture-in-Picture', 'so-ssl' )
			),
			'publickey-credentials-get'       => array(
				'default'     => 'self',
				'description' => __( 'Controls access to WebAuthn API', 'so-ssl' )
			),
			'screen-wake-lock'                => array(
				'default'     => 'self',
				'description' => __( 'Controls access to Wake Lock API', 'so-ssl' )
			),
			'sync-xhr'                        => array(
				'default'     => 'self',
				'description' => __( 'Controls the ability to use synchronous XHR', 'so-ssl' )
			),
			'usb'                             => array(
				'default'     => 'self',
				'description' => __( 'Controls access to USB devices', 'so-ssl' )
			),
			'web-share'                       => array(
				'default'     => 'self',
				'description' => __( 'Controls access to the Web Share API', 'so-ssl' )
			),
			'xr-spatial-tracking'             => array(
				'default'     => 'self',
				'description' => __( 'Controls access to WebXR features', 'so-ssl' )
			)
		);
